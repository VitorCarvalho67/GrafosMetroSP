...
\begin{abstract}
  ...
  world in space-L ($\sigma=0.51$), is dominated by the high-centrality stations
  Luz and Brás, and
...
\begin{resumo}
  ...
  livre de escala \emph{nem} de mundo pequeno no espaço L ($\sigma=0{,}51$), é
  dominada pelas estações de alta centralidade Luz e Brás e colapsa sob ataques
  dirigidos. Modeladas do mesmo
...
caráter de mundo pequeno, poucas estações de alta centralidade e distribuições
...
cerca de $393$~km, $15$ linhas e $185$ estações~\cite{metrosp2026,cptm2026}.
...
independentes. Reservamos, por isso, o termo \emph{hub} para o sentido de rede
livre de escala e designamos as estações mais conectadas ou centrais como
\emph{estações de alta centralidade}.
...
NetworkX~\cite{hagberg2008}. Como camada complementar de exploração,
desenvolvemos visualizações interativas em \emph{Three.js}/WebGL\footnote{\url{https://threejs.org}}:
um grafo 3D navegável da malha, um explorador com planejador de rota e mapa de
calor, a comparação lado a lado das três redes e um globo 3D com São Paulo,
Londres e Nova York. Código, dados, as visualizações e instruções de reprodução
estão em \url{https://github.com/VitorCarvalho67/GrafosMetroSP}.

% =====================================================================
\section{Resultados}
% =====================================================================

\paragraph{Estrutura (Q1--Q2).} A malha é conexa, com $N=185$, $M=198$ e
$\langle k\rangle=2{,}14$ (Tabela~\ref{tab:metricas}). O grau máximo é $6$ (Luz)
e $79\%$ das estações têm grau $2$: não há cauda longa, logo a rede \emph{não} é
livre de escala. O comprimento característico é alto ($L=14{,}0$; $D=37$) e a
eficiência global $E_{\mathrm{glob}}=0{,}109$. Comparando a $300$ grafos
aleatórios, $C\approx C_{\mathrm{rand}}$ mas $L\gg L_{\mathrm{rand}}$, dando
$\sigma=0{,}51<1$: no espaço L a rede \emph{não} é de mundo pequeno. Essa
propriedade só surge no espaço P ($L_P=2{,}61$, $D_P=6$; $\approx 1{,}6$ baldeação
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lr lr@{}}
    \toprule
    Estações $N$ & 185 & Agrupamento $C$ & 0{,}009 \\
    Trechos $M$ & 198 & Mundo pequeno $\sigma$ & 0{,}51 \\
    Grau médio $\langle k\rangle$ & 2{,}14 & Modularidade $Q$ & 0{,}817 \\
    Compr.\ caract.\ $L$ & 14{,}05 & Comunidades & 14 \\
    Diâmetro $D$ & 37 & Robustez $r_T$ & 0{,}076 \\
    Eficiência $E_{\mathrm{glob}}$ & 0{,}109 & Baldeações & 23 \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Estações de alta centralidade (Q1).} As estações de maior intermediação
(Figura~\ref{fig:hubs}) são Brás e Luz (ambas com $C_B\approx 0{,}44$), seguidas de
...
$Q=0{,}817$ (alto, típico de redes em corredores), com claro sentido geográfico.
Usando a proximidade média como acessibilidade regional, o centro expandido
(Luz, Brás, República, Barra Funda; $\overline{C_C}=0{,}104$) é cerca de
...
muito vulnerável à perda de poucas estações de altíssima conectividade.
...
Paulo ($0{,}076$ vs.\ $0{,}080$) e Londres ($0{,}143$ vs.\ $0{,}140$), validando
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}lrrrrrr@{}}
    ...
    \midrule
    São Paulo & 185 & 2{,}14 & 0{,}009 & \textbf{0{,}51} & 0{,}124 & 0{,}076 \\
...
em $+0{,}65\%$. Modelando linhas em projeto pelas baldeações anunciadas
